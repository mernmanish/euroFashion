            ...
            <div class="card">
              <div class="card-body">
                ...
                <table class="table table-bordered" id="table">
                 ...
                 <?php
                 $sql=$this->db->select('*')->from('bulk_order')->order_by('id','desc')->get()->result_array();
                 $i=1;
                 foreach($sql as $rows)
                 { 
                 ...
                 ?>
                    ...
                    <?php
                      if($rows['status']==1){  
                      ?>
                    <td><a href="<?php echo base_url(); ?>cp/Order/bulk_product/<?php echo $rows['id']; ?>" class="btn btn-info btn-sm">Add Product</a></td>
                  <?php } else { ?>
                    <td></td>
                  <?php } ?>
                    </tr>
    ...
    var abc=status;
     ...
     else
     {
      ...
      var formdata={'id':id,'status':status};
       $.ajax({
          url: '<?php echo base_url('cp/Order/changebulkorder'); ?>',
          type: 'POST',
          dataType: 'json',
          data: formdata,
