...
<div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1>Blog List <a href="<?php echo base_url(); ?>cp/Blog/blog_details?do=new" class="btn btn-success" style="float: right;">Add Blog Details</a></h1>
          </div>
        </div> 
      </div>
    </section>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <table class="table table-bordered" id="table">
                  ...
                  <?php
                  $sqluser=$this->db->select('blog.*,blog_category.name as categoryname')->from('blog')->join('blog_category','blog_category.id=blog.cat_id')->order_by('id','DESC')->get()->result_array();
                  $i=1;
                   ...
                   ?>
                    <tr>
                     ...
                     <?php } else { ?>
                      ...
                      <td>
		               <a  class="btn btn-warning btn-sm" href="<?php echo base_url(); ?>cp/Blog/blog_details?do=update&id=<?php echo $rows['id']; ?>"><i class="fa fa-pencil"></i></a>
		              <button class="btn btn-danger btn-sm" id="<?php echo $rows['id']; ?>" onclick="deletenews(this.id)"><i class="fa fa-trash"></i></button></td>
                    </tr>
                  <?php $i++; } ?>
                  </tbody>
                </table>
...
<?php else: ?>
	<div class="content-wrapper">
     <section class="content-header">
      <h1>Add/Update Blog Details <a href="<?php echo base_url(); ?>cp/Blog/blog_details" class="btn btn-success pull-right">Blog List</a></h1>
    </section>
    <section class="content">
      <div class="container-fluid">
        <div class="row">
            ...
            <div class="card card-primary">
              <!-- <div class="card-header">
                <h3 class="card-title">Update User</h3>
		          ...
		          <?php $id=$_GET['id']; 
              $sqledit=$this->db->select('*')->from('blog')->where('id',$id)->get()->result_array();
                ?>
               <form role="form" id="newsform" method="post">
                <div class="card-body">
                  <div class="row">
                  <div class="form-group col-md-4">
                    <input type="hidden" name="newsid" id="newsid" value="<?php echo $sqledit[0]['id']; ?>">
                    <label for="cat_id">Category Name</label>
                    <select class="form-control" name="cat_id" id="cat_id">
                      <option value="">Select Category</option>
                      <?php $sqlcount=$this->db->select('*')->from('blog_category')->order_by('id','DESC')->get()->result_array(); 
                       foreach($sqlcount as $rows)
                       {
                      ?>
                      <option value="<?php echo $rows['id']; ?>" <?php if($sqledit['0']['cat_id']==$rows['id']){ echo "selected"; } ?>><?php echo $rows['name']; ?></option>
                    <?php } ?>
                    </select>
                  </div>
                  <div class="form-group col-md-4">
                    <label for="news_title">Blog Title</label>
                    <input type="text" class="form-control" id="news_title" name="news_title" placeholder="Enter Blog Title" value="<?php echo $sqledit[0]['news_title']; ?>">
                  </div>
              ...
              <form role="form" id="newsform" method="post">
                <div class="card-body">
                  ...
                  <div class="form-group col-md-4">
                    <label for="news_title">Blog Title</label>
                    <input type="text" class="form-control" id="news_title" name="news_title" placeholder="Enter Blog Title">
                  </div>
                  <div class="col-md-4 form-group">
                    <label for="image">Image</label>
                    <div class="input-group">
                    ...
                    <span class="icon-error" style="display: none; font-weight: bold;"></span>
                  </div>
                </div>
                <div class="row">
                  <div class="form-group col-md-12">
...
<script type="text/javascript">
    ...
    submitHandler: function() {
      ...
      if($.inArray(ext, ['png','jpg','jpeg','gif','']) == -1) {
        $('.icon-error').show().addClass('text-danger').text("Please Upload Image Only");
        $('.icon-error').fadeOut(3000);
     }
     else{
      ...
      $.ajax({
                url: '<?php echo base_url(); ?>cp/Blog/managenews', 
                type: 'POST',
                dataType: 'json', 
                data: formData,
                contentType: false,
            ...
            .done(function(data) {
               ...
               $('.alert').show().addClass('alert-primary').text("Data Added Successfully");
                ...
                $('.alert').fadeOut(6000);
                 $('#newsform .btn-success').text("Submit Details");
              } 
            }
            else{
  ...
  function deletenews(id)
  {
    ...
    var formData={"id":id};
    $.ajax({
      url:'<?php echo base_url('cp/Blog/deletenews'); ?>',
      type:'POST',
      dataType:'json',
      data: formData,
