...
<div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1>Location List <a href="<?php echo base_url(); ?>branch/Location_master/location?do=new" class="btn btn-success" style="float: right;">Add New Location</a></h1>
          </div>
        </div> 
      </div>
    </section>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <table class="table table-bordered" id="table">
                  ...
                  <?php
                  $sqluser=$this->db->select('location.*,state.name as statename,city.name as cityname')->from('location')->join('state','state.id=location.state_id')->join('city','city.id=location.city_id')->where('location.branch_id',$this->session->userdata('sessionuser')['userid'])->order_by('location.id','DESC')->get()->result_array();
                  $i=1;
                  foreach($sqluser as $rows)
                  {
                   ?>
                    <tr>
                      ...
                      <td>
		               <a  class="btn btn-warning btn-sm" href="<?php echo base_url(); ?>branch/Location_master/location?do=update&id=<?php echo $rows['id']; ?>"><i class="fa fa-pencil"></i></a>
		              <button class="btn btn-success btn-sm" id="<?php echo $rows['id']; ?>" onclick="deletelocation(this.id)"><i class="fa fa-trash"></i></button></td>
                    </tr>
                  <?php $i++; } ?>
                  </tbody>
                </table>
...
<?php else: ?>
	<div class="content-wrapper">
     <section class="content-header">
      <h1>Add/Update Location <a href="<?php echo base_url(); ?>branch/Location_master/location" class="btn btn-success pull-right">Location List</a></h1>
    </section>
    <section class="content">
      <div class="container-fluid">
        <div class="row">
                  ...
                  <div class="form-group col-md-3">
                   ...
                   <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="city_id" id="city_id">
                      <option value="">Select State</option>
                      <?php $sqlcount=$this->db->select('*')->from('city')->where('branch_id',$this->session->userdata('sessionuser')['userid'])->order_by('name','ASC')->get()->result_array(); 
                       foreach($sqlcount as $rows)
                       {
                      ?>
...
<script type="text/javascript">
    ...
    submitHandler: function() {
      ...
      $.ajax({
                url: '<?php echo base_url(); ?>branch/Location_master/managelocation', 
                type: 'POST',
                dataType: 'json', 
                data: formData,
                contentType: false,
  ...
  function deletelocation(id)
  {
    ...
    var formData={"id":id};
    $.ajax({
      url:'<?php echo base_url('branch/Location_master/deletelocation'); ?>',
      type:'POST',
      dataType:'json',
      data: formData,
...
<script type="text/javascript">
    ...
    $('#state_id').change(function(){
      ...
      {
       $.ajax({
        url:"<?php echo base_url(); ?>branch/Location_master/changelocation",
        method:"POST",
