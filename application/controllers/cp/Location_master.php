...
class Location_master extends CI_Controller
{
   ...
   {
   	$this->load->view('cp/state');
   }
   public function managestate()
   {
   	$id=$this->input->post('stateid');
    $name=$this->input->post('name');
    $count=$this->db->select('*')->where('name',$name)->where('id<>',$id)->from('state')->count_all_results();
     if ($count>0) {
       $data['success']=true;
        $data['status']="avail";
     }else{
   ...
   public function managecity()
   {
    ...
    $count=$this->db->select('*')->where('state_id',$state_id)->where('name',$name)->where('id<>',$id)->from('city')->count_all_results();
     if ($count>0) {
       $data['success']=true;
        $data['status']="avail";
     }else{
   ...
   public function managelocation()
   {
    ...
    $count=$this->db->select('*')->where('state_id',$state_id)->where('city_id',$city_id)->where('name',$name)->where('id<>',$id)->from('location')->count_all_results();
     if ($count>0) {
       $data['success']=true;
        $data['status']="avail";
     }else{
  ...
  public  function changebranchcity()
  {
    $branch_id=$this->input->post('branch_id');
    $sqlsize=$this->db->select('*')->where('branch_id',$branch_id)->get('state')->result();
    $sqlsize=json_decode(json_encode($sqlsize),true);
    $output='<option value="">Select State</option>';
    foreach($sqlsize as $rows)
    {
