...
class Product extends CI_Controller
{
  public function __construct()
  {
     ...
     $this->load->model('Exceldb');
  }
   public function index()
   {
   	 $this->load->view('cp/product');
   }
  ...
  public function manage_product()
   ...
   public function changestocktatus()
   {
    ...
    print_r($stockData);
    if($this->db->where('id',$pro_id)->update('product',$stockData))
    {
      $data['success']=true;
    }
    else
  ...
  /*->like('product.brandid',$brandid)->like('product.categoryid',$categoryid)*/
   ...
   {
    $this->load->view('cp/import-product');
   }
        ...
        foreach($file_data as $row)
          {
           ...
           $cat = $this->db->like('name', $row["Category"])->get('category')->result_array();
           $brand = $this->db->like('name', $row["Brand"])->get('brand')->result_array();
           $dataS = array(
            'vendor_id' => $this->input->post('vendor_id'),
            'sku_no' => $row["SKU"],
            ...
            'sale_type'   => $row["Total Stock"],
            'ctime' => date('Y-m-d H:i:s'),
            'cuid'=> $this->session->userdata('sessionuser')['id'],
            'muid'=> $this->session->userdata('sessionuser')['id']
           );
           $proData = $this->db->insert('product',$dataS);
          }
          $data['success'] =true;
          
          echo json_encode($data);
      // $import = $this->Exceldb->import_excel_item($data);
   }
